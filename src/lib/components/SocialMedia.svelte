<script lang="ts">
    ...
    $: if (browser) {url = window.location.origin + '/event/' + event.id }
...
<svelte:head>
    ...
    <meta property='og:url' content="//tli.xyvid.com/{event.id}" /> <!-- # dynamic url capturing doesn't work here, need SSR solution -->
</svelte:head>
